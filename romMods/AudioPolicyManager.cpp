    ...
    if (availablePrimaryOutputDevices() & rxDevice) {
        ...
        if (output != AUDIO_IO_HANDLE_NONE) {
            ...
            if (activeInput != 0) {
                ...
                stopInput(activeInput, activeDesc->getActiveAudioSessions().keyAt(0));
                releaseInput(activeInput, activeDesc->getActiveAudioSessions().keyAt(0));
            }
    ...
    if (createTxPatch) {
        ...
        if (activeInput != 0) {
            ...
            if (activeDesc->getModuleHandle() == txSourceDeviceDesc->getModuleHandle()) {
                audio_session_t activeSession = activeDesc->getActiveAudioSessions().keyAt(0);
                                         ...
                                         audio_policy_forced_cfg_t config)
{
    ...
    if (activeInput != 0) {
        ...
        if (activeDesc->mProfile->getSupportedDevicesType() & (newDevice & ~AUDIO_DEVICE_BIT_IN)) {
